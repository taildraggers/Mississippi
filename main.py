"""Aggregates Mississippi-located listings from every taildraggers.com
...
ones whose location ends in ", MS".
...
Minnesota repos' approach, with the state filter swapped to "MS".
"""
...
PAGE_TITLE = "Taildragger Ads in Mississippi"
STATE_ABBR = "MS"
...
_MISSISSIPPI_LOCATION_RE = re.compile(r",\s*MS\s*$", re.IGNORECASE)


def _is_mississippi(location: str) -> bool:
    return bool(_MISSISSIPPI_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        mississippi_rows = [row for row in rows if _is_mississippi(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(mississippi_rows)} in Mississippi")
        all_listings.extend(mississippi_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Mississippi listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Mississippi listings: {len(listings)}")
