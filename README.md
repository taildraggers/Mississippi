...
Daily aggregator of Mississippi-located taildragger classified
...
repos' approach, with the state filter swapped to "MS".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MS` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Mississippi"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/mississippi/`
...
```html
<iframe
  src="https://taildraggers.github.io/mississippi/"
  title="Taildragger Ads in Mississippi"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MISSISSIPPI_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
